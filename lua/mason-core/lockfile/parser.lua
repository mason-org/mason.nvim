...
local function main()
    ...
    if shit.header then
        for _, key in ipairs(_.sort_by(_.identity, _.keys(shit.header))) do
            output[#output + 1] = serialize(0, key, shit.header[key])
        end
        output[#output + 1] = { "", "---", "" }
    end

    for _, key in ipairs(_.sort_by(_.identity, _.keys(shit.body))) do
        output[#output + 1] = serialize(0, key, shit.body[key])
        output[#output + 1] = { "" }
    end
